...
| 3. **Feed-primary + watch-targeted reconcile** (3-clock hybrid; NOT feed-only) | 🟡 **In progress (2026-07-16)** — Clocks 1/2a/2b/3 ✅ code-done, cron ✅ wired behind `FEED_INGEST_ENABLED` (off), prod cutover + blind-path retirement ⬜ | Feed alone misses ~5% of genuinely-bookable slots (stale records). Keep a scoped scrape as a correctness backstop; retire only the *blind fixed-cadence* scrape + proxy-heavy path. Clock 2b bounding resolved: **bounded round-robin**. |
...
3. **Feed-primary + watch-targeted reconcile (3-clock hybrid).** 🟡 In progress (2026-07-16). Stand up the three clocks (see "Feed reliability + hybrid ingestion" above):
   ...
   - **Cron wiring:** ✅ **done** — `api/cron/scrape/route.ts` branches on `FEED_INGEST_ENABLED`; when on, `runFeedIngest()` runs Clock 1 (every tick) + Clock 2b (throttled `RECONCILE_INTERVAL_MIN`) + Clock 3 (throttled `SWEEP_INTERVAL_HOURS`), unions transitions → one `notifyUsers`. Default off = legacy scrape (no regression). **Cutover TODO (prod-ops):** populate `courts`, reset `slots` to feed-owned rows, set `FEED_INGEST_ENABLED=true`.
   - **Retire the blind path:** ⬜ delete `scrape-scheduler.ts` + `scrape_targets` (fixed per-venue-date cadence). **Keep `scrapers/courtside.ts`** (now the reconcile fetcher) and `cheerio`. Demote proxy to the low-rate Clock 2/3 backstop (proxy env optional, not required). Add the 3 bonus TH venues to active tracking.
