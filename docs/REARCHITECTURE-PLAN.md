...
| 7. Contract 0.2.0 + website + map | 🟡 **Backbone done (2026-07-23)** — contract 0.2.0 (additive Venue geo/metadata + AvailabilitySlot startsAt/endsAt/remainingUses/bookingUrl + expo-push DTO); `/api/venues` now DB-sourced (enriched, resolves the code-vs-DB split); `/api/availability` exposes the new slot fields. Publish tag `contract-v0.2.0` ⬜, map view ⬜, expanded venue list ⬜ | Additive → 0.1.0 clients still compile. PostGIS deferred → map uses plain lat/lng bbox. Map lib/scope still to decide. |
