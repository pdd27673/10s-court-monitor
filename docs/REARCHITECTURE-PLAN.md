...
| 3. **Feed-primary + watch-targeted reconcile** (3-clock hybrid; NOT feed-only) | 🟡 **In progress (2026-07-16)** — Clock 1 ✅ code-done (unwired), Clock 2a+2b ✅ code-done (unwired), Clock 3 ⬜, cron wiring + cutover ⬜ | Feed alone misses ~5% of genuinely-bookable slots (stale records). Keep a scoped scrape as a correctness backstop; retire only the *blind fixed-cadence* scrape + proxy-heavy path. Clock 2b bounding resolved: **bounded round-robin**. |
...
3. **Feed-primary + watch-targeted reconcile (3-clock hybrid).** 🟡 In progress (2026-07-16). Stand up the three clocks (see "Feed reliability + hybrid ingestion" above):
   ...
   - **Clock 3** — daily full sweep (24h): ⬜ scrape all venue-days; dashboard floor + feed-drop net; re-baselines Clock 2.
   - **Cron wiring + cutover:** ⬜ wire Clock 1 + Clock 2b + Clock 3 into the cron (throttled), flip `persist:true` once the feed owns `slots`.
