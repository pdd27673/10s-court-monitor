...
| 3. **Feed-primary + watch-targeted reconcile** (3-clock hybrid; NOT feed-only) | ⬜ **Reworked 2026-07-12** — see "Feed reliability" below | Feed alone misses ~5% of genuinely-bookable slots (stale records). Keep a scoped scrape as a correctness backstop; retire only the *blind fixed-cadence* scrape + proxy-heavy path. |
...
3. **Feed-primary + watch-targeted reconcile (3-clock hybrid).** Stand up the three clocks (see "Feed reliability + hybrid ingestion" above):
   - **Clock 1** — feed head-poll loop (30s): flip `ingestSlots({persist:true})`, apply RPDE deltas to `slots`, notify on transition. Populate `courts`/geo first (`ingestFacilities()` in prod).
   - **Clock 2** — watch-targeted reconcile (15min): from active watches compute the *pending set* (watched (venue,date,time) currently `taken` in DB); scrape only those venue-day pages via `scrapeCourtside`; **site wins**; site-available/DB-taken → update + notify.
