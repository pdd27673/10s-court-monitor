...
- **Phase 3 — feed head-poll (Clock 1): DONE, WIRED, persist:true.** `pollSlots({persist:true})` delta-poller in `ingest/openactive/ingest.ts` — resumes from `feed_state` cursor (first run backfills page 1), applies RPDE deltas to `slots`, detects booked/closed→available via `isNewlyAvailable`, returns `SlotChange[]`. Now emits an `unresolvedBy` breakdown (foreign / unmapped-court / non-tennis-excluded / no-time / bad-data) so national-feed noise ≠ a seeding gap. Non-tennis courts are **seeded and flagged** (`courts.non_tennis`) instead of dropped — they resolve into `excludedNonTennis` (never written, never notified) so they can't be mistaken for an unmapped court. A real unmapped court **self-heals**: those slots are buffered, the facility ingest re-runs (throttled, `FACILITY_HEAL_MIN_MINUTES`), and they're retried in the same tick. Preview: `scripts/poll-slots-preview.ts`.
...
- **Map view DONE** — `/map` page (`src/app/map/page.tsx`) + `src/components/map/VenueMap.tsx` (Leaflet + OpenStreetMap raster tiles, one pin per venue with coords — green courtside / blue clubspark — popups with address + booking link). Client-only via `next/dynamic` `ssr:false`; self-contained SVG `divIcon` (no external asset, no CSP change — `img-src https:` already covers OSM tiles). "Map" link added to `SiteNav`. Deps: `leaflet` + `react-leaflet@5` + `@types/leaflet`. Verified with a Playwright screenshot (2 mock venues → 2 markers, no page errors; tiles are blank only because the sandbox can't reach OSM — they load on staging).
- Gates green (152 tests, typecheck + contract, lint, build).
- **Remaining:** publish the contract (tag `contract-v0.2.0`); optionally wire the expanded DB venue list + "updated Ns ago" into the dashboard (the 0.2.0 API already exposes it). ⚠️ Heads-up: `npm audit` shows 2 **high** (not critical) advisories in `sharp`/libvips (Next image optimizer, newly-disclosed 2026 CVEs) — unrelated to the map deps; CI's prod-critical gate still passes. Clear with `npm audit fix` when convenient.
