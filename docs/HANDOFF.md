...
**Phase 5 — split the worker service (IN PROGRESS this session).** Extract the ingest loop into `src/lib/ingest/worker.ts` as a 2nd Railway service entrypoint (same repo, different start command) sharing `DATABASE_URL`; trim the web service to API+UI; retire the external cron (keep a manual re-backfill trigger). Single-instance worker so polling stays exactly-once.
