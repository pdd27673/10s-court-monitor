...
- **Phase 3 cron wiring: DONE (behind a cutover flag).** `src/app/api/cron/scrape/route.ts` now branches on `FEED_INGEST_ENABLED` (default off → legacy blind scrape, zero regression). When on, `runFeedIngest()` runs Clock 1 every tick + Clock 2b (throttled `RECONCILE_INTERVAL_MIN`, default 15) + Clock 3 (throttled `SWEEP_INTERVAL_HOURS`, default 24), each failure-isolated, unions transitions → one `notifyUsers` call. Per-clock throttles use `feed_state` source=`clock`. **Cutover (still TODO, prod-ops):** populate `courts` in prod (`ingestFacilities()`/let facility cron do it), reset `slots` to feed-owned rows, then set `FEED_INGEST_ENABLED=true`. Reversible: unset the flag to fall back to the scraper.
