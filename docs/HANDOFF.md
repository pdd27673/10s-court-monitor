...
> cold session / Cursor can pick up. Last updated: 2026-07-12.
...
- **Clock 2 — watch-targeted reconcile:** ⚠️ **PARTIALLY DONE + design revised by a budget finding.**
  ...
  - **⚠️ BUDGET FINDING (2026-07-12):** ran the preview against the 12 live watches → **79 venue-days/run ≈ 7,584 pages/day at 15-min cadence — 3× today's ~2,400 blind scrapes.** The "watch-targeted = cheap" premise (docs said ~400–750/day) is WRONG for the current watch mix. Two causes: (1) **half-migrated prod state** — `courts` EMPTY, `external_id` null on all venues (⇒ `ingestFacilities()` never ran in prod; rearchitecture undeployed), and 4 watched venues (abbotts-park, king-edward-memorial-park, poplar-rec-ground, wapping-gardens) have **0 slots** (scraper doesn't cover them) + st-johns-park closed (0 available) ⇒ ~40 of 79 pending venue-days are **phantom** (missing/closed rows count as pending); (2) **structural** — one all-venues watch × 10 venues × 8 days makes raw pending scale with venue-count × watch-breadth.
  - **⇒ Clock 2b (site-wins fetch+upsert+notify) must be BOUNDED, not full-pending-every-15-min.** Design decision pending with user: bounded round-robin (fixed `RECONCILE_MAX_PAGES`/run, rotate least-recently-checked → cost independent of watch growth, miss-latency = budget-derived) vs lower cadence vs full. MUST also exclude closed (st-johns) + dead (abbotts) + zero-data venues, and stop treating a missing row as pending once the feed owns slots. **A trustworthy budget can't be measured until the feed owns the slots table (post-cutover), because today's number is inflated by data gaps.**
  - 2b also needs canonical court-label mapping (scraper "Tennis court N" → feed "Court N") so it writes the feed-owned rows instead of creating duplicates. Belongs with the cutover (Edit 3).
