...
- **Phase 3 Clock 2b (watch-targeted reconcile — fetch/upsert/notify): CODE DONE (built + green, UNWIRED).** `reconcileWatchedVenueDays()` in `src/lib/ingest/reconcile.ts` — **bounded round-robin** (design decision resolved with user 2026-07-16: fixed `RECONCILE_MAX_PAGES`/run, rotate least-recently-checked pending venue-days via `feed_state` source=`reconcile`). Site-wins upsert into feed-owned rows with canonical court labels; returns `SlotChange[]` for `notifyUsers`. `persist` defaults FALSE (same cutover gate as Clock 1). Preview: `scripts/reconcile-run-preview.ts`. Still TODO: wire into cron + flip `persist:true` at cutover; needs prod `courts` populated + feed owning `slots` for a real transition signal (pre-cutover previews mechanics only).
